...
function createTimestamp(urlObject, timestamp) {
	...
	timestampObject.save({
		...
		success: function success(timestampObject) {
			parseObject = timestampObject;
		},

		error: function error(urlObject, _error) {
			alert("Error while creating a new Timestamps instance: " + _error.code + " " + _error.message);
		}
	});
}
...
function captureTab(tab) {
	...
	new Parse.Query('Urls').equalTo("url", tab.url).first({
		...
		success: function success(urlObject) {
			if (urlObject != undefined) {
				...
				urlObject.save({
					...
					error: function error(urlObject, _error3) {
						alert("Error while creating a new Urls instance: " + _error3.code + " " + _error3.message);
					}
				});
			}
		},
		...
		error: function error(_error2) {
			alert("Error while querying Url: " + _error2.code + " " + _error2.message);
		}
	});
}
...
function releaseCurrentTab(timestamp) {
	...
	parseObject.save(null, {
		success: function success(siteAccess) {
			// The object was saved successfully.
		},
		error: function error(siteAccess, _error4) {
			// The save failed.
			// error is a Parse.Error with an error code and message.
			alert("Error while saving the released object: " + _error4.code + " " + _error4.message);
		}
	});
...
chrome.tabs.onActivated.addListener(function (activeInfo) {
	...
	chrome.tabs.get(activeInfo.tabId, function (activatedTab) {
		...
		if (activatedTab.url == null) {
			chrome.tabs.onUpdated.addListener(function onActivatedUpdatedHandler(tabId, changeInfo, updatedTab) {
...
},{"parse":3,"parseuri":4}],2:[function(require,module,exports){
